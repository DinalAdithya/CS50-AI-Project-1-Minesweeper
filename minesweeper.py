...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for x in range(cell[0] - 1, cell[0] + 2):
            for y in range(cell[1] - 1, cell[1] + 2):
                if (x, y) not in self.moves_made and (x, y) not in self.mines and (x, y) not in self.safes:
    ...
    def update_knowledge(self):
        # Check each sentence in knowledge base
        for sentence in self.knowledge:
    ...
    def make_random_move(self):
        ...
        all_moves = [(x, y) for x in range(self.width) for y in range(self.height)
                     if (x, y) not in self.moves_made and not self.mines]
